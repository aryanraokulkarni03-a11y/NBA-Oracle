...
| Phase 5 execution closeout | Complete | The dashboard now has a Guide route, shared explanation mappings, human-readable reasons, bookmaker/provider naming cleanup, inline metric explanations, page-level interpretation panels, and advanced-only raw details on prediction cards. | The product now reads like an operator tool instead of an internal engineering surface | Revisit only if future Phase 7 model changes add new concepts that need explanation |
...
| Phase 6 final one-shot planning | Complete | Phase 6 is now rewritten as one ordered operating closeout covering launcher, recurring scheduler automation, workflow truth, tunnel recovery, and evidence-loop reliability in one pass. | A cleaner execution target for the final ops pass with less improvisation and fewer partial fixes | Execute the Phase 6 implementation work end to end |
| Phase 6 execution closeout | Complete | Hosted launcher scripts, scheduler helper scripts, startup sanity coverage, verified Windows task scheduling, and a dedicated Phase 6 runbook are now landed. | A real operator path for recurring scheduling and daily operation with less guesswork | Keep the free local dashboard workflow documented as the default daily path |
| Free-first workflow clarification | Complete | The docs now treat local backend + local dashboard + recurring scheduler as the recommended fully free daily workflow, while Vercel + quick Cloudflare Tunnel remains optional hosted access only. | An honest operator story without pretending quick tunnels are stable primary infrastructure | Revisit hosted guidance later only if a stable named tunnel or paid backend host exists |
| Phase 7 split planning | Complete | Phase 7 is now split into 7A forecast visibility and 7B intelligence upgrade so lower-risk operator-surface work lands before deeper model changes. | A cleaner sequence for the next major product and model pass | Execute 7A first, then 7B |
| Phase 7A execution closeout | Complete | Today now shows actionable upcoming games plus a next-up lookahead, and Performance now includes predicted-versus-actual truth with overall, BET, and LEAN accuracy summaries. | A much more useful operator-facing planning and review surface without changing the core model logic yet | Follow through with the deeper intelligence pass in 7B |
| Phase 7B execution closeout | Complete | The predictor now blends a market-aware prior with richer injury/stats context, timing adjustments, and uncertainty-aware gating; learning/reporting now understand segments and uncertainty; replay acceptance still passes. | A materially smarter pregame moneyline engine without breaking the existing operator workflow | Revisit only when we choose a new future expansion phase |
| Full project audit | Complete | A one-shot audit now exists documenting what is complete, what is intentionally deferred, which commits mattered most, and what operational truths still apply. | A cleaner closeout record for the current product scope | Keep it updated only when a new expansion phase actually lands |
...
| Ops tooling | Hosted launcher batch file, backend/tunnel helper scripts, scheduler runner script, scheduler registration script, Phase 6 runbook, and verified scheduler task | Keep the local dashboard workflow as the default free daily operating path |
| Dashboard | React/Vite scaffold, top-nav shell, authenticated pages, operator action cards, shared CSS system, hosted API-base support, dashboard-wide runtime refresh, actionable Today sections, next-up lookahead, predicted-versus-actual truth inside Performance, and Phase 7B reason translation coverage | Keep UI polish synced with backend truth later |
